...
const configure = async (cmdObj) => {
  ...
  const configFunctionsMapping = {
    ...
    webflowTableReference: configureWebflowTableReference,
  ...
  if (cmdObj.all) {
    ...
    await configureWebflowTableReference()
  ...
  async function configureWebflowTableReference() {
    const input = await inquirer.askWebflowTableReference()
    configurer.set('webflowTableReference', input.webflowTableReference)
    if (configurer.getWebflowTableReference()) console.log('Webflow Original Table Reference is set 👍')
  }
